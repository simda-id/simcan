...
CREATE TABLE IF NOT EXISTS `90_ref_sub_unit` (
  ...
  KEY `idx_ref_sub_unit` (`id_unit`,`kd_sub`,`status_data`) USING BTREE,
...
) ENGINE=InnoDB AUTO_INCREMENT=1472 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=COMPACT;
...
) ENGINE=InnoDB AUTO_INCREMENT=197 DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci ROW_FORMAT=COMPACT;
...
CREATE TABLE `13_vref_kegiatan` (
	...
	`kode_program` VARCHAR(17) NOT NULL COLLATE 'utf8mb4_general_ci',
	`id_kegiatan` INT(11) NOT NULL,
	`kd_kegiatan` INT(11) NOT NULL,
	`uraian_kegiatan` VARCHAR(255) NULL COLLATE 'utf8mb4_unicode_ci',
	`kode_kegiatan` VARCHAR(21) NOT NULL COLLATE 'utf8mb4_general_ci'
) ENGINE=MyISAM;
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY DEFINER VIEW `13_vref_kegiatan` AS select `a`.`kd_urusan` AS `kd_urusan`,`a`.`kode_urusan` AS `kode_urusan`,`a`.`nm_urusan` AS `nm_urusan`,`b`.`id_bidang` AS `id_bidang`,`b`.`kd_bidang` AS `kd_bidang`,`b`.`nm_bidang` AS `nm_bidang`,concat(`a`.`kd_urusan`,'.',right(concat('0',`b`.`kd_bidang`),2)) AS `kode_bidang`,`c`.`id_program` AS `id_program`,`c`.`kd_program` AS `kd_program`,`c`.`uraian_program` AS `uraian_program`,concat(`a`.`kd_urusan`,'.',right(concat('0',`b`.`kd_bidang`),2),'.',right(concat('0',`c`.`kd_program`),2)) AS `kode_program`,`d`.`id_kegiatan` AS `id_kegiatan`,`d`.`kd_kegiatan` AS `kd_kegiatan`,`d`.`nm_kegiatan` AS `uraian_kegiatan`,concat(`a`.`kd_urusan`,'.',right(concat('0',`b`.`kd_bidang`),2),'.',right(concat('0',`c`.`kd_program`),2),'.',right(concat('00',`d`.`kd_kegiatan`),3)) AS `kode_kegiatan` from (((`ref_urusan` `a` join `ref_bidang` `b` on((`a`.`kd_urusan` = `b`.`kd_urusan`))) join `ref_program` `c` on((`b`.`id_bidang` = `c`.`id_bidang`))) join `ref_kegiatan` `d` on((`c`.`id_program` = `d`.`id_program`)));
