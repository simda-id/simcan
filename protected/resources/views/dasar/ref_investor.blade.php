...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <?php
                $this->title = ' Jumlah Investor PMDN/PMA  ';
    ...
    <div role="tabpanel" class="tab-pane fade in active" id="investor">
                  <div class="col-md-12">
                  <div class="add">
                      <button id="btnAddinvestor" type="button" class="btn btn-labeled btn-sm btn-success"><span class="btn-label"><i class="glyphicon glyphicon-plus"></i></span>Tambah Jumlah Investor</button>
                      <button id="btnPrintinvestor" type="button" class="btn btn-labeled btn-sm btn-primary"><span class="btn-label"><i class="glyphicon glyphicon-print"></i></span>Print Jumlah Investor</button>
                  </div>
                  <div class="">
                  <table id="tblinvestor" class="table table-striped table-bordered table-responsive compact" width="100%" cellspacing="0">
                        <thead>
                            <tr>
 ...
 <div id="Modalinvestor" class="modal fade" role="dialog" data-backdrop="static">
  <div class="modal-dialog modal-lg"  >
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form name="frmModalinvestor" class="form-horizontal" role="form" autocomplete='off' action="" method="post" onsubmit="return false;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
<!--             <input type="hidden" name="id_pemda" id="id_pemda"> -->
<!--             <input type="hidden" name="id_investor" id="id_investor"> -->
            <div class="row">
            <div class="col-sm-2" style="text-align: center;">
              <i class="fa fa-suitcase fa-fw" style="font-size: 100px;color: #357EBD;"></i>
            </div>
            <div class="col-sm-10">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-10 text-right">
                      <div class="ui-group-buttons">
                        <button type="button" class="btn btn-sm btn-success btninvestor btn-labeled" data-dismiss="modal">
...
<div id="ModalHapusinvestor" class="modal fade" role="dialog" data-backdrop="static">
  <div class="modal-dialog modal-lg"  >
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"></h4>
      ...
      <div class="modal-body">
        <form name="frmModalHapusinvestor" class="form-horizontal" role="form" autocomplete='off' action="" method="post" onsubmit="return false;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
<!--             <input type="hidden" name="id_pemda" id="id_pemda"> -->
<!--             <input type="hidden" name="id_investor" id="id_investor"> -->
            <div class="row">
        ...
        <div class="modal-footer">
                <div class="row">
                    ...
                    <div class="col-sm-10 text-right">
                      <div class="ui-group-buttons">
                        <button type="button" class="btn btn-sm btn-success btnHapusinvestor btn-labeled" data-dismiss="modal">
...
$(document).ready(function() {
	//alert('A');
var investor_tbl=$('#tblinvestor').DataTable({
                  processing: true,
                  serverSide: true,
                  dom : 'BfRtip',                  
                  autoWidth : false,
                  "ajax": {"url": "./investor/getListinvestor"},
                  "language": {
                      "decimal": ",",
                      "thousands": "."},
                  "columns": [
...
$(document).on('click', '#btnAddinvestor', function() {
	  $('.btninvestor').removeClass('edit');
	  $('.btninvestor').addClass('add');
	  $('.modal-title').text('Tambah Jumlah Investor PMDN/PMA');
...
// 	  text_tahun_1.style.visibility = 'hidden';
	  ...
	  $.ajax({
          type: "GET",
          url: './investor/getKecamataninvestor',
          dataType: "json",
          success: function(data) {
...
$( ".kecamatan" ).change(function() {
    ...
    $.ajax({
        ...
        type: "GET",
        url: './investor/getSektorinvestor/'+$('#tahun').val()+'/'+$('#kecamatan').val(),
        dataType: "json",
...
$('.modal-footer').on('click', '.add', function() {
    ...
    $.ajax({
        ...
        success: function(data) {
              $('#tblinvestor').DataTable().ajax.reload();
...
//edit function
$(document).on('click', '#btnEditinvestor', function() {
//alert("1");

var data = investor_tbl.row( $(this).parents('tr') ).data();

  $('.btninvestor').removeClass('add');
  $('.btninvestor').addClass('edit');
  $('.modal-title').text('Edit Jumlah Investor PMDN/PMA');
  $('.form-horizontal').show();
  $('#Modalinvestor').modal('show');
...
$(document).on('click', '#btnHapusinvestor', function() {
	var data = investor_tbl.row( $(this).parents('tr') ).data();
	$('.btnHapusinvestor').removeClass('add');
	  $('.btnHapusinvestor').addClass('remove');
	  $('.modal-title').text('Hapus Jumlah Investor PMDN/PMA');
	  $('.form-horizontal').show();
	 $('#ModalHapusinvestor').modal('show');
...
$(document).on('click', '#btnPrintinvestor', function() {

	//alert("print");
     window.open('./Printinvestor/2018');
