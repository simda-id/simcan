...
<div class="container-fluid">
    <div class="row">
        <div class="col-md-12">
            <?php
                $this->title = ' Nilai dan Kontribusi Sektor dalam PDRB berdasarkan Harga Berlaku  ';
    ...
    <div role="tabpanel" class="tab-pane fade in active" id="pdrbhb">
                  <div class="col-md-12">
                  <div class="add">
                      <button id="btnAddpdrbhb" type="button" class="btn btn-labeled btn-sm btn-success"><span class="btn-label"><i class="glyphicon glyphicon-plus"></i></span>Tambah PDRB</button>
                      <button id="btnPrintpdrbhb" type="button" class="btn btn-labeled btn-sm btn-primary"><span class="btn-label"><i class="glyphicon glyphicon-print"></i></span>Print PDRB</button>
                  </div>
                  <div class="">
                  <table id="tblpdrbhb" class="table table-striped table-bordered table-responsive compact" width="100%" cellspacing="0">
                        <thead>
                            <tr>
 ...
 <div id="Modalpdrbhb" class="modal fade" role="dialog" data-backdrop="static">
  <div class="modal-dialog modal-lg"  >
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form name="frmModalpdrbhb" class="form-horizontal" role="form" autocomplete='off' action="" method="post" onsubmit="return false;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
<!--             <input type="hidden" name="id_pemda" id="id_pemda"> -->
<!--             <input type="hidden" name="id_pdrbhb" id="id_pdrbhb"> -->
            <div class="row">
            <div class="col-sm-2" style="text-align: center;">
              <i class="fa fa-newspaper-o fa-fw" style="font-size: 100px;color: #357EBD;"></i>
            </div>
            <div class="col-sm-10">
                ...
                <div class="row">
                    ...
                    <div class="col-sm-10 text-right">
                      <div class="ui-group-buttons">
                        <button type="button" class="btn btn-sm btn-success btnpdrbhb btn-labeled" data-dismiss="modal">
...
<div id="ModalHapuspdrbhb" class="modal fade" role="dialog" data-backdrop="static">
  <div class="modal-dialog modal-lg"  >
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title"></h4>
      ...
      <div class="modal-body">
        <form name="frmModalHapuspdrbhb" class="form-horizontal" role="form" autocomplete='off' action="" method="post" onsubmit="return false;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
<!--             <input type="hidden" name="id_pemda" id="id_pemda"> -->
<!--             <input type="hidden" name="id_pdrbhb" id="id_pdrbhb"> -->
            <div class="row">
        ...
        <div class="modal-footer">
                <div class="row">
                    ...
                    <div class="col-sm-10 text-right">
                      <div class="ui-group-buttons">
                        <button type="button" class="btn btn-sm btn-success btnHapuspdrbhb btn-labeled" data-dismiss="modal">
...
$(document).ready(function() {
	//alert('A');
var pdrbhb_tbl=$('#tblpdrbhb').DataTable({
                  processing: true,
                  serverSide: true,
                  dom : 'BfRtip',                  
                  autoWidth : false,
                  "ajax": {"url": "./pdrbhb/getListpdrbhb"},
                  "language": {
                      "decimal": ",",
                      "thousands": "."},
                  "columns": [
...
$(document).on('click', '#btnAddpdrbhb', function() {
	  $('.btnpdrbhb').removeClass('edit');
	  $('.btnpdrbhb').addClass('add');
	  $('.modal-title').text('Tambah Kontribusi PDRB berdasarkan Harga Berlaku');
	  $('.form-horizontal').show();
	  $('#Modalpdrbhb').modal('show');
	  $('#nama_kolom').val("");
	  $('#kecamatan').val("");
	  $('#nmin1').val("");
	  $('#nmin2').val("");
...
$( ".kecamatan" ).change(function() {
    ...
    $.ajax({
        ...
        type: "GET",
        url: './pdrbhb/getSektorpdrbhb/'+$('#tahun').val()+'/'+$('#kecamatan').val(),
        dataType: "json",
...
$('.modal-footer').on('click', '.add', function() {
    ...
    $.ajax({
        ...
        success: function(data) {
              $('#tblpdrbhb').DataTable().ajax.reload();
...
//edit function
$(document).on('click', '#btnEditpdrbhb', function() {
//alert("1");

var data = pdrbhb_tbl.row( $(this).parents('tr') ).data();

  $('.btnpdrbhb').removeClass('add');
  $('.btnpdrbhb').addClass('edit');
  $('.modal-title').text('Edit Kontribusi PDRB berdasarkan Harga Berlaku');
  $('.form-horizontal').show();
  $('#Modalpdrbhb').modal('show');
...
$(document).on('click', '#btnHapuspdrbhb', function() {
	var data = pdrbhb_tbl.row( $(this).parents('tr') ).data();
	$('.btnHapuspdrbhb').removeClass('add');
	  $('.btnHapuspdrbhb').addClass('remove');
	  $('.modal-title').text('Hapus Kontribusi PDRB berdasarkan Harga Berlaku');
	  $('.form-horizontal').show();
	 $('#ModalHapuspdrbhb').modal('show');
...
$(document).on('click', '#btnPrintpdrbhb', function() {

	//alert("print");
     window.open('./PrintPdrbHb/2018');
