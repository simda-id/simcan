      ...
      @foreach($data as $datas)
        ...
        'children': [
            @foreach($datas->TrxRpjmdMisis as $misi)
                {'name': 'Misi RPJMD {{ $misi->no_urut }}','title': '{{ $misi->uraian_misi_rpjmd}}','className': 'level2',
                'children': [
                    @foreach($misi->TrxRpjmdTujuans as $tujuan)
                        {'name': 'Tujuan RPJMD {{ $tujuan->no_urut }}','title': '{{ $tujuan->uraian_tujuan_rpjmd}}','className': 'level3',
                        'children': [
                            @foreach($tujuan->TrxRpjmdSasarans as $sasaran)
                                {'name': 'Sasaran RPJMD {{ $sasaran->no_urut }}','title': '{{ $sasaran->uraian_sasaran_rpjmd}}','className': 'level4',
                                'children': [
                                    @foreach($sasaran->TrxRenstraSasarans as $sasrenstra)
                                        {'name': 'Sasaran Renstra','title': '{{ $sasrenstra->uraian_sasaran_renstra}}',
                                        'children': [
                                            @foreach($sasrenstra->TrxRenstraPrograms as $program)
                                                {'name': 'Program Renstra','title': '{{ $program->uraian_program_renstra}}','className': 'level5',
                                                'children': [
                                                    @foreach($program->TrxRenstraKegiatans as $kegiatan)
                                                        {'name': 'Kegiatan Renstra','title': '{{ $kegiatan->uraian_kegiatan_renstra}}',},
                                                    @endforeach
                                                ]},
                                            @endforeach
                                        ]},
