$( document ).ready( function () {
  ...
  $( document ).on( 'click', '.btnUpdateItemSSH', function () {
    ...
    $( '.form-horizontal' ).show();
    $( '#nama_tarif_update' ).val( data.ur_tarif );
    $( '#id_tarif_perkada_update' ).val( data.id_tarif_perkada );
    $( '#ModalUpdateItem900' ).modal( 'show' );
  } );

  $( '.modal-footer' ).on( 'click', '.btnUpdateItem900', function () {
    $.ajaxSetup( {
      headers: { 'X-CSRF-Token': $( 'meta[name=_token]' ).attr( 'content' ) }
    } );
    ...
    $.ajax( {
      ...
      data: {
        '_token': $( 'input[name=_token]' ).val(),
        'id_tarif_perkada': $( '#id_tarif_perkada_update' ).val(),
        'id_dokumen_keu': $( '#no_dokumen_update' ).val(),
      },
      success: function ( data ) {
        $( '#ModalUpdateItem900' ).modal( 'hide' );
        if ( data.status_pesan == 1 ) {
          createPesan( data.pesan, "success" );
        } else {
          createPesan( data.pesan, "alert" );
